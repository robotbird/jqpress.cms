...
namespace Jqpress.Framework.DbProvider
{
    ...
    static partial class SqlMapper
    {
        ...
        private static IDbCommand SetupCommand(IDbConnection cnn, IDbTransaction transaction, string sql, Action<IDbCommand, object> paramReader, object obj, int? commandTimeout, CommandType? commandType)
        {
            ...
            //comment by yepeng for oracle or mysql paramters
            //if (cnn.GetType() == typeof(System.Data.OracleClient.OracleConnection))
            //    sql = sql.Replace("@", ":").Replace("::", "@@").Replace("?", ":");
            //else if (cnn.GetType() == typeof(MySql.Data.MySqlClient.MySqlConnection))
            //    sql = sql.Replace("@", "?").Replace("??", "@@").Replace(":", "?");
        ...
        ///// <returns></returns>
        //private string GetParameterFormat(IDbCommand command)
        //{
        //    if (!System.Data.SqlClient.  ParametersFormat.ContainsKey(command.GetType()))
        //    {
